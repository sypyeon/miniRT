...
/*   Updated: 2025/07/26 19:36:41 by sipyeon          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static _Bool	solve_cylinder_equation(t_obj *cy, t_ray *ray, t_hit_record *rec);
...
static _Bool	solve_cylinder_equation(t_obj *cy, t_ray *ray, t_hit_record *rec)
{
    ...
    oc_perp = vminus(oc, vscale(axis, vdot(oc, axis)));
    a = vdot(ray_perp, ray_perp);
    return (calculate_roots(cy, ray, rec, ray_perp, oc_perp, a));
}
...
static _Bool	calculate_roots(t_obj *cy, t_ray *ray, t_hit_record *rec)
{
    ...
    ray_perp = vminus(ray->dir, vscale(axis, vdot(ray->dir, axis)));
    oc_perp = vminus(oc, vscale(axis, vdot(oc, axis)));
    discriminant = calculate_discriminant(ray_perp, oc_perp, cy);
    if (discriminant < 0)
        return (0);
    return (test_cylinder_roots(cy, ray, rec));
}

static double	calculate_discriminant(t_vec ray_perp, t_vec oc_perp, t_obj *cy)
{
    double	a;
    double	b;
    double	c;
...
static _Bool	test_cylinder_roots(t_obj *cy, t_ray *ray, t_hit_record *rec)
{
    t_vec	axis;
    t_vec	oc;
    t_vec	ray_perp;
    ...
    ray_perp = vminus(ray->dir, vscale(axis, vdot(ray->dir, axis)));
    oc_perp = vminus(oc, vscale(axis, vdot(oc, axis)));
    t = calculate_first_root(ray_perp, oc_perp, cy);
    if (test_single_root(cy, ray, rec, t))
        return (1);
    t = calculate_second_root(ray_perp, oc_perp, cy);
    return (test_single_root(cy, ray, rec, t));
}

static double	calculate_first_root(t_vec ray_perp, t_vec oc_perp, t_obj *cy)
{
    double	a;
    double	b;
    double	discriminant;
...
static double	calculate_second_root(t_vec ray_perp, t_vec oc_perp, t_obj *cy)
{
    double	a;
    double	b;
    double	discriminant;
...
static _Bool	test_single_root(t_obj *cy, t_ray *ray, t_hit_record *rec, double t)
{
    t_point	hit_point;
...
static _Bool	record_cylinder_hit(t_obj *cy, t_ray *ray, t_hit_record *rec, double t)
{
    t_vec	axis;
    t_point	hit_point;
    double	projection;
